...
지금부터 웹 검색으로 최신 시장 데이터를 모아, 아래 스키마에 맞는 JSON을 `./data.json` 파일에 **딱 하나** 작성한다.
...
3. **오직 `./data.json` 파일만 작성**한다. 채팅 출력은 한 줄 요약이면 충분하다. 다른 파일은 건드리지 마라.
...
- 최종 산출물은 **유효한 JSON**이어야 한다 (주석·트레일링 콤마 금지). `./data.json`에 저장.
